...
/* 마른 붓 반점 타일 — 캔버스 돌기에 물감이 안 앉은 아주 작은 흰 점(획 몸통용).
 * ⚠️ endStroke 후처리 금지 — 펜을 떼는 순간 구멍이 "펑" 나타나는 팝인으로 읽힌다
 * (2026-07-06 사용자 실측). GL은 dab 셰이더에서, 2D는 presentStroke 라이브 경로에서
 * 실시간 침식한다(프리뷰=최종). 고정 시드 + 캔버스 좌표 고정(identity transform) —
 * 종이 결(v_px 샘플)과 같은 "종이에 붙은" 모델이라 색 게이트 결정론 유지. */
let fleckTileC: HTMLCanvasElement | null = null;

export function fleckTile(): HTMLCanvasElement {
...
/** 획 버퍼에 마른 붓 흰 점 침식(destination-out) — Canvas2D 폴백의 라이브/최종 공용 */
