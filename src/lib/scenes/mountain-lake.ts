...
import {
  ...
  createBird, createButterfly, createRock, createGroup, createLine, createPolygon,
...
function createDock(x: number, y: number, length: number, width: number, rng: SeededRandom): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 2, category: 'dock', modifiable: true, id: uid('dock') });
}
...
function createBoat(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 3, category: 'boat', modifiable: true, id: uid('boat') });
}
...
function createDeer(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 3, category: 'animal', modifiable: true, id: uid('deer') });
}
...
function createDistantCabin(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 1, category: 'cabin', modifiable: false, id: uid('cabin') });
}
...
export function generateMountainLake(rng: SeededRandom, w: number, h: number): { elements: SVGElementData[]; defs: string } {
  ...
  const defs = `
    ...
    <linearGradient id="lakeGrad" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="#1976D2" stop-opacity="0.85"/>
      <stop offset="30%" stop-color="#0097A7" stop-opacity="0.75"/>
      <stop offset="60%" stop-color="#00838F" stop-opacity="0.7"/>
      <stop offset="100%" stop-color="#006064" stop-opacity="0.65"/>
    </linearGradient>
    ...
    <radialGradient id="sunGlow" cx="50%" cy="50%" r="50%">
      ...
      <stop offset="25%" stop-color="#FFD54F" stop-opacity="0.8"/>
  ...
  // Outer glow
  elements.push(createCircle(sunX, sunY, 50, 'url(#sunGlow)', { layer: 0, category: 'sun', modifiable: false, opacity: 0.8, id: uid('sun_glow') }));
  // Inner sun
  elements.push(createCircle(sunX, sunY, 22, '#FFD54F', { layer: 0, category: 'sun', modifiable: true, opacity: 0.95, id: uid('sun') }));

  // === CLOUDS (4-6) WITH SHADOWS ===
  ...
  for (let i = 0; i < cloudCount; i++) {
    ...
    elements.push(createCloud(cx, cy, cs, rng));
  ...
  for (let i = 0; i < mtnCount; i++) {
    ...
    const depth = (i + 1) / mtnCount; // nearest = darkest
    const lightness = 35 + (1 - depth) * 25;
    const mColor = `hsl(210, 15%, ${lightness}%)`;

    // Mountain body
    const mPts = `${mx},${h * 0.4} ${mx + mw / 2},${h * 0.4 - mh} ${mx + mw},${h * 0.4}`;
    elements.push(createPolygon(mx, h * 0.4, mPts, mColor, { layer: 0, category: 'mountain', modifiable: false, opacity: 0.6 + depth * 0.35, id: uid('mtn') }));

    // Ridgeline detail (subtle texture lines)
    const ridgeY = h * 0.4 - mh * 0.6;
    elements.push(createLine(mx + mw * 0.3, ridgeY + mh * 0.1, mx + mw * 0.5, h * 0.4 - mh, '#546E7A', { strokeWidth: 1, layer: 0, opacity: 0.25 + depth * 0.1 }));
    elements.push(createLine(mx + mw * 0.5, h * 0.4 - mh, mx + mw * 0.7, ridgeY + mh * 0.1, '#546E7A', { strokeWidth: 1, layer: 0, opacity: 0.25 + depth * 0.1 }));

    // Snow on peaks
    const snowW = mw * 0.3;
    const snowPts = `${mx + mw / 2 - snowW / 2},${h * 0.4 - mh * 0.7} ${mx + mw / 2},${h * 0.4 - mh} ${mx + mw / 2 + snowW / 2},${h * 0.4 - mh * 0.7}`;
    elements.push(createPolygon(mx, h * 0.4, snowPts, '#F5F5F5', { layer: 0, category: 'mountain', modifiable: false, opacity: 0.85, id: uid('mtn_snow') }));

    // Reflected mountain silhouette in lake (inverted, semi-transparent)
    const refPts = `${mx},${h * 0.4} ${mx + mw / 2},${h * 0.4 + mh * 0.5} ${mx + mw},${h * 0.4}`;
    elements.push(createPolygon(mx, h * 0.4, refPts, mColor, { layer: 1, category: 'reflection', modifiable: false, opacity: 0.12 + depth * 0.05, id: uid('mtn_ref') }));
  }
  ...
  // Add pebbles along bottom shore explicitly
  ...
  // ============================
  // === LAYER 2: TREES (6-10) pine and oak mix ===
  // ============================
  ...
  for (let i = 0; i < treeCount; i++) {
    ...
    // Tree reflection in water (simple inverted ellipse, muted)
    if (ty < h * 0.6) {
      const refY = h * 0.4 + (h * 0.4 - (ty - treeSize * 0.3)) * 0.4 + h * 0.08;
      elements.push(createEllipse(tx, refY, treeSize * 0.12, treeSize * 0.25, '#2E7D32', {
        layer: 1, category: 'reflection', modifiable: false, opacity: 0.08, id: uid('tree_ref'),
      }));
    }
  }
  ...
  for (let i = 0; i < rng.nextInt(3, 5); i++) {
    ...
    const treeType = rng.pick(['pine', 'oak'] as const);
    elements.push(createTree(tx, ty, treeType, rng.nextFloat(80, 140), rng));
  }

  // ============================
  // === LAYER 2: WILDFLOWERS (6-10) ALONG SHORE ===
  // ============================
  for (let i = 0; i < rng.nextInt(6, 10); i++) {
    ...
    elements.push(createFlower(fx, fy, flowerType, rng.nextFloat(15, 32), rng));
  }

  // ============================
  // === LAYER 2: ROCKS (4-7) BY WATER ===
  // ============================
  for (let i = 0; i < rng.nextInt(4, 7); i++) {
    const rx = rng.nextFloat(w * 0.1, w * 0.85);
    const ry = rng.nextFloat(h * 0.7, h * 0.82);
    elements.push(createRock(rx, ry, rng.nextFloat(12, 35), rng));
  }
  ...
  // ============================
  // === LAYER 4: BIRDS (2-4) SOARING ===
  // ============================
  for (let i = 0; i < rng.nextInt(2, 4); i++) {
    elements.push(createBird(rng.nextFloat(40, w - 40), rng.nextFloat(25, h * 0.3), true, rng));
  }
  ...
  for (let i = 0; i < rng.nextInt(2, 3); i++) {
    elements.push(createButterfly(rng.nextFloat(w * 0.05, w * 0.95), rng.nextFloat(h * 0.6, h * 0.85), rng.nextFloat(12, 20), rng));
  }
