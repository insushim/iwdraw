...
import {
  ...
  createLine, createPolygon, createTree, createStar, createFence,
...
function christmasTree(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  // Star glow
  children.push(createCircle(x, starY, s * 0.08, '#FFD700', { layer: 3, opacity: 0.3 }));

  // Ornaments (colored circles on the tree)
  ...
  for (let i = 0; i < rng.nextInt(10, 16); i++) {
    ...
    const or = rng.nextFloat(s * 0.015, s * 0.03);
    children.push(createCircle(ox, oy, or, rng.pick(ornColors), { layer: 3, opacity: 0.9 }));
    // Highlight
    children.push(createCircle(ox - or * 0.3, oy - or * 0.3, or * 0.3, '#FFF', { layer: 3, opacity: 0.5 }));
  }
  ...
  return createGroup(x, y, children, { layer: 2, category: 'christmastree', modifiable: true, id: uid('xtree') });
}
...
function cozyHouse(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  for (const win of winPositions) {
    ...
    const wh = s * 0.12;
    // Warm glow behind
    children.push(createRect(win.wx - s * 0.02, win.wy - s * 0.02, ww + s * 0.04, wh + s * 0.04, '#FFD700', { layer: 2, opacity: 0.15 }));
    // Window
  ...
  // String lights on eaves (colorful dots)
...
function candyCane(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 3, category: 'candycane', modifiable: true, id: uid('candy') });
}
...
function giftBox(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 3, category: 'gift', modifiable: true, id: uid('gift') });
}
...
export function generateChristmasScene(rng: SeededRandom, w: number, h: number): { elements: SVGElementData[]; defs: string } {
  ...
  const defs = `
    <linearGradient id="nightSkyGrad" x1="0" y1="0" x2="0" y2="1">
      ...
      <stop offset="40%" stop-color="#142850"/>
      <stop offset="70%" stop-color="#1A3366"/>
      <stop offset="100%" stop-color="#223D6E"/>
    </linearGradient>
  ...
  bStarD += 'Z';
  elements.push(createPath(bStarX, bStarY, bStarD, '#FFD700', { layer: 0, category: 'star', modifiable: true, id: uid('bigstar'), opacity: 0.95 }));
  ...
  // === LAYER 1: Snow ground (wavy white surface) ===
  ...
  for (let i = 0; i < pineCount; i++) {
    ...
    const tree = createTree(tx, groundY + rng.nextFloat(-5, 5), 'pine' as const, ts, rng);
    elements.push(tree);

    // Snow on tree branches
    for (let j = 0; j < 3; j++) {
      const snowW = ts * (0.25 - j * 0.05);
      const snowY = groundY - ts * (0.1 + j * 0.25) + rng.nextFloat(-5, 5);
      elements.push(createEllipse(tx, snowY, snowW, ts * 0.03, '#FFF', {
  ...
  // Window warm glow (spreading out)
  elements.push(createEllipse(houseX - h * 0.16, groundY - h * 0.12, h * 0.12, h * 0.08, 'url(#warmGlow)', { layer: 1, category: 'glow', modifiable: false }));
  elements.push(createEllipse(houseX + h * 0.08, groundY - h * 0.12, h * 0.12, h * 0.08, 'url(#warmGlow)', { layer: 1, category: 'glow', modifiable: false }));

  // === LAYER 2: Second smaller house (optional) ===
  if (rng.chance(0.6)) {
  ...
  elements.push(createFence(fenceX, groundY + 3, fenceLen, rng));
  // Snow on fence
  ...
  // A few snowflake shapes (6-pointed star-like)
  for (let i = 0; i < rng.nextInt(5, 10); i++) {
    ...
    elements.push(createGroup(sx, sy, children, { layer: 4, category: 'snowflake', modifiable: false, opacity: rng.nextFloat(0.4, 0.8) }));
  }
