...
import {
  ...
  lampGlowGradient, groundGradient, softGlow, dropShadow, starfieldPattern,
  turbulenceTexture,
} from './svg-effects';
import {
  detailedTree, detailedRock, detailedStar, detailedFence,
  resetComponentId,
} from './svg-components';
...
function carvedPumpkin(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 3, category: 'pumpkin', modifiable: true, id: uid('pumpkin'), filter: 'url(#shadow)' });
}
...
function tombstone(x: number, y: number, size: number, rng: SeededRandom, showRIP: boolean): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 3, category: 'tombstone', modifiable: true, id: uid('tomb'), filter: 'url(#shadow)' });
}
...
function ghost(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  children.push(createPath(x, y, bodyPath, '#F5F5F5', { layer: 4, opacity: 0.5, filter: 'url(#glow)' }));
...
function blackCat(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 3, category: 'cat', modifiable: true, id: uid('cat'), filter: 'url(#shadow)' });
}
...
function cauldron(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  children.push(createEllipse(x, y - s * 0.15, s * 0.25, s * 0.05, '#00FF00', { layer: 3, opacity: 0.6, filter: 'url(#potionGlow)' }));
  ...
  return createGroup(x, y, children, { layer: 3, category: 'cauldron', modifiable: true, id: uid('cauldron'), filter: 'url(#shadow)' });
}
...
function hauntedHouse(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 2, category: 'building', modifiable: true, id: uid('hauntedhouse'), filter: 'url(#shadow)' });
}

// Dead tree (gnarled, no leaves)
function deadTree(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  return createGroup(x, y, children, { layer: 2, category: 'tree', modifiable: true, id: uid('deadtree'), filter: 'url(#shadow)' });
}
...
function pathLantern(x: number, y: number, size: number, rng: SeededRandom): SVGElementData {
  ...
  children.push(createCircle(x, y - s * 0.54, s * 0.12, 'url(#lanternGlow)', { layer: 3, opacity: 0.5 }));

  return createGroup(x, y, children, { layer: 3, category: 'lantern', modifiable: true, id: uid('lantern'), filter: 'url(#shadow)' });
}
...
export function generateHalloweenNight(rng: SeededRandom, w: number, h: number): { elements: SVGElementData[]; defs: string } {
  ...
  const defs = combineDefs(
    ...
    radialGradient('moonGlow', [
      { offset: '0%', color: '#FFFACD', opacity: 0.95 },
      { offset: '40%', color: '#F0E68C', opacity: 0.6 },
      { offset: '70%', color: '#DAA520', opacity: 0.15 },
      { offset: '100%', color: '#DAA520', opacity: 0 },
    ]),
    radialGradient('pumpkinGlow', [
      { offset: '0%', color: '#FF8C00', opacity: 0.5 },
      { offset: '60%', color: '#FF6600', opacity: 0.15 },
      { offset: '100%', color: '#FF6600', opacity: 0 },
    ]),
    lampGlowGradient('lanternGlow', '#FF8C00'),
    softGlow('potionGlow', 6, '#00FF00'),
    softGlow('fogGlow', 12, '#888888'),
    groundGradient('darkGround', '#0A1A0A', '#050F05'),
    turbulenceTexture('fogTexture', 0.02, 2, 6),
  );
  ...
  // Detailed twinkling stars
  for (let i = 0; i < rng.nextInt(3, 6); i++) {
    elements.push(detailedStar(rng.nextFloat(20, w - 20), rng.nextFloat(15, h * 0.35), rng.nextFloat(2, 4), rng));
  }
  ...
  elements.push(createCircle(moonX, moonY, moonR, '#FFFACD', { layer: 0, category: 'moon', modifiable: true, id: uid('moon'), opacity: 0.95, filter: 'url(#glow)' }));
  // Craters
  for (let i = 0; i < rng.nextInt(4, 7); i++) {
  ...
  // === LAYER 1: Ground ===
  const groundPathD = `M0,${groundY} Q${w * 0.15},${groundY - 8} ${w * 0.3},${groundY + 3} Q${w * 0.5},${groundY - 5} ${w * 0.7},${groundY + 4} Q${w * 0.85},${groundY - 3} ${w},${groundY + 2} L${w},${h} L0,${h} Z`;
  elements.push(createPath(0, 0, groundPathD, 'url(#darkGround)', { layer: 1, category: 'ground', modifiable: false }));
  ...
  elements.push(detailedFence(graveStartX - 10, groundY + 3, tombCount * w * 0.08 + 30, 1, rng));
