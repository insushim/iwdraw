...
test("최소 굵기에서도 획이 끊기지 않고 부드럽다", async ({ page }) => {
  ...
  for (const name of BRUSHES) {
    ...
    const stat = await page.evaluate(() => {
      ...
      /* ⚠️ 윗변은 **서브픽셀로** 잡는다(정수 행 번호 금지).
       * 정수 행으로 재면 잉크가 임계(235)를 넘는 지점이 픽셀 경계 근처인 열에서 값이
       * 통째로 1px 튄다. 그러면 dab 위치 지터(Math.random, 색연필 ±0.02px)만으로도
       * 일부 열이 무작위로 뒤집혀 고주파 잔차가 실행마다 0.00~0.25를 오간다
       * (2026-07-29 실측: 같은 코드로 색연필 0.00 / 0.08 / 0.25 — 게이트 0.2를 넘었다 말았다).
       * 임계 전후 두 행의 잉크량으로 교차점을 선형 보간하면 지터는 0.02px 그대로 남고
       * 진짜 톱니(dab마다 1px 오르내림)는 그대로 잡힌다 = 신호/잡음이 크게 벌어진다. */
      const inkAt = (x: number, y: number) => {
        const i = (y * w + x) * 4;
        return 255 - Math.min(line[i], line[i + 1], line[i + 2]); // 0(흰 종이)~255
      };
      const THR = 20; // painted()의 <235와 같은 임계
      const rows: number[][] = [];
      for (let x = 0; x < w; x++) {
        const edges: number[] = [];
        ...
        for (let y = 0; y < hLine; y++) {
          ...
          if (p && !prevPainted) {
            const b = inkAt(x, y);
            const a = y > 0 ? inkAt(x, y - 1) : 0;
            // a < THR <= b 구간에서 임계를 넘는 지점
            edges.push(b > a ? y - (b - THR) / (b - a) : y);
          }
          prevPainted = p;
        }
        edges.forEach((e, i) => {
  ...
  for (const [name, s] of Object.entries(report)) {
     ...
     * dab 무작위 회전을 얇은 획에서 되살리면(ROT_JITTER_MIN_PX=0) 연필 0.24 · 사인펜 0.33으로
     * 게이트를 넘는다(2026-07-29 서브픽셀 윗변 기준 실측) = 원래 잡으려던 결함은 그대로 잡힌다.
     * 정상 코드의 잡음은 3회 반복에서 0.01~0.04로 안정 — 신호/잡음 8배.
     * (정수 윗변 시절에는 잡음이 실행마다 0.00~0.25라 신호와 겹쳤다. 위 서브픽셀 주석 참조.)
     *
